...
def new():
    ...
    data_m=np.zeros((nx, ny))
    data_b=np.zeros((nx, ny))
    data_A=np.zeros((nx, ny))
    data_status=np.zeros((nx, ny))
    data_norm=np.zeros((nx, ny))
    ...
    fits.writeto('norm_full.fits', data = data_norm, overwrite = True)
